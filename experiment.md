...
> **Status: pre-registered design document (written 2026-05, extended 2026-05-21 with §d.7 Phase 2.1; project closed 2026-09-02).**
> This is the plan and its rationale, not the results — every prediction, novelty claim and decision rule below was written before the corresponding data existed. What actually ran and what it found: [`docs/FINAL_STATUS.md`](docs/FINAL_STATUS.md). Two of the plan's expectations did not survive the data: the Phase-D D-gate on MLA came out **negative** (D3/D4/E1 were never commissioned), and the Part-I "gain needs RoPE" reading was replaced by "gain needs per-head Q–K geometry" (`FINAL_STATUS.md` §4.5–4.6). Note that §d.7 (Phase 2.1) and the archived §d.7-legacy (Phase 2) both use sub-section numbers d.7.1–d.7.6; the archived block is the older plan. `suggestion.md`, cited throughout §d.7 and in several configs, was a private Phase-2 review memo that was never committed; treat those citations as provenance only — the plan of record is §d.7 and the outcome of record is `docs/FINAL_STATUS.md`.
...
- "Hyperparameter Transfer Enables Consistent Gains of Matrix-Preconditioned Optimizers Across Scales" (Qiu, Chen, Phan, Lei, Wilson — arXiv 2512.05620, NeurIPS 2025): "Muon, SOAP and Shampoo consistently achieve near 1.4× speedup over AdamW for training Llama-architecture language models of sizes ranging from 190M to 1.4B" when combined with μP-style 1/width independent weight decay; the speedup is uniform across the three matrix-preconditioned optimizers (not a Muon-vs-Shampoo differential).
...
- "Hyperparameter Transfer …" (Qiu, Chen, Phan, Lei, Wilson — arXiv 2512.05620, NeurIPS 2025): "Muon, SOAP and Shampoo consistently achieve near **1.4× speedup** over AdamW for training Llama-architecture language models of sizes ranging from 190M to 1.4B" when combined with μP-style 1/width independent weight decay (uniform across the three optimizers; not a Muon-vs-Shampoo differential).
...
- If it drops between **C and B** → the RoPE block reshape was crucial; investigate whether the partial-RoPE / no-RoPE Q–K coupled formula is even well-defined. Note: the implementation auto-falls-back to flat-2D Q–K coupling whenever `pos_emb.type=learned` *or* `rope_partial_frac<1.0` (no per-head 2-D rotation pair exists in those cases), so C/D/E and any partial-RoPE rung silently lose the multi-head Q-K branch. *(Superseded 2026-09-02: Phase-2.1 B0 replaced this warn-and-disable with the `optimizer.qk_coupling` three-flag dispatcher (§d.7.3), so flat-2D is now only the default policy; and per `docs/FINAL_STATUS.md` §4.5 the C/D regression is mostly an LR-window/probe-flagging artefact — per-head `headwise_no_rope` coupling beats Muon on C/D/E at own-best LR.)*
...
- Hold weight decay = 0.1 / width (Qiu et al., arXiv 2512.05620 §3.4 finding); β₁ = 0.95, β₂ = 0.95 for AdamW; β = 0.95, ns_steps = 5, coupled_steps = 4 for Coupled Muon.
...
5. **Weight decay matched.** Use 1/width-scaled weight decay (Qiu et al. 2025, arXiv 2512.05620) for both optimizers. WD on Muon and WD on AdamW are *not* the same regulariser (Muon WD acts on already-orthogonal updates), so don't directly transfer numbers.
...
- **The "≈ 50 % wall-clock" Muon-vs-AdamW figure is the literature upper bound.** Realistic, well-tuned reference at 60M–500M dense is 1.10–1.50× (Essential AI 2505.02222: 10–15 %; Wen et al. 2509.02046: ~1.3× under 520M; Qiu et al. 2512.05620: 1.4× across 190M–1.4B). Calibrate before claiming a coupled-Muon-vs-Muon delta on top.
...
> **Historical plan (frozen 2026-07-13).** Phases A, B, C and D all ran; the D-gate came out negative, so D3/D4/E1 were correctly never commissioned. The "active sweep list" in `DISPATCH.md` is frozen too. Readout: `docs/FINAL_STATUS.md` §4.2–4.6.
