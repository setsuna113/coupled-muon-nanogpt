...
The Coupled-vs-AdamW gap of $0.12$--$0.29$\nats{} translates, via the standard log-linear loss-vs-tokens calibration at this scale, to an effective $\sim$1.3--1.5$\times$ token-budget speedup for the Muon-family pair over \optAdamW{}. The Coupled-vs-Muon gap of $0.01$--$0.024$\nats{} corresponds to roughly $\sim$1.02--1.04$\times$ additional token-budget --- a small fixed-token loss improvement at this scale, and the right order of magnitude to ask whether it survives Stage~3's sparse MoE rungs. It should not be presented as a wall-clock win: at each arm's own best LR, \optCoupled{}'s median runtime is about $1.04$--$1.07\times$ \optMuon{} on the dense rungs, so the measured runtime overhead is comparable to or larger than the token-efficiency estimate.
